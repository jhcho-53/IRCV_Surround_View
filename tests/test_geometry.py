...
cams = load_cameras()
if os.path.exists(f"{config.FLOOR_BOARD_DIR}/Cam_001"):
    g = geometry.fit_ground_plane(cams)                       # re-fit from the floor recording
    ...
    tag = "fit rms=%.1fcm" % vf["rms_cm"]
else:
    g = geometry.load_ground(); vf = geometry.load_vehicle()  # bundled artifacts (clone-only)
    assert abs(np.linalg.norm(g["n"]) - 1) < 1e-6
    tag = "bundled"
